...
@Component({
  ...
  template: `
    <div class="form-control" [class.has-error]=
    ...
    (group?.errors && (group?.dirty || group?.touched))">
      ...
      >
        ...
        <fa-icon [icon]="errorIcon"> </fa-icon> Este campo é obrigatorio
        </p>
        <p class="error-message" *ngIf="control.hasError('malformedMail')">
        <fa-icon [icon]="errorIcon"> </fa-icon> O correo introducido non é válido
        </p>
      </ng-container>
      ...
      <p 
        ...
        *ngIf="control.valid && group.hasError('passwordMismatch')">
        <fa-icon [icon]="errorIcon"> </fa-icon> O contrasinal e a confirmación deben ser iguais
        </p>
      </ng-container>
    </div>`
